...
namespace UserControl_uniform_piece_make
  {
  public partial class TWebUserControl_uniform_piece_make: ki_web_ui.usercontrol_class
    {
    private struct p_type
      ...
      public bool be_ok_to_config_uniform_piece_makes;
      public string id;
    ...
    protected void Page_Load(object sender, System.EventArgs e)
      ...
      if (!p.be_loaded)
        {
        LinkButton_new_record.Visible = p.be_ok_to_config_uniform_piece_makes;
    ...
    private bool PresentRecord(string id)
      ...
      if
        ...
        SetDependentFieldAblements(p.be_ok_to_config_uniform_piece_makes);
        Button_submit.Enabled = p.be_ok_to_config_uniform_piece_makes;
        Button_delete.Enabled = p.be_ok_to_config_uniform_piece_makes;
        result = true;
        }
      return result;
      }
    ...
    protected override void OnInit(System.EventArgs e)
      ...
      if (Session[instance_id + ".p"] != null)
        ...
        //
        if (instance_id == "ASP.protected_overview_aspx.UserControl_M_config_business_objects_binder_uniform_object_binder_uniform_piece_make")
          {
          p.be_loaded &= ((Session["M_UserControl_config_UserControl_business_objects_binder_UserControl_uniform_object_binder_PlaceHolder_content"] as string) == "UserControl_uniform_piece_make");
          }
//      else if (instance_id == "ASP.~_aspx.UserControl_~_binder_uniform_piece_make")
//        {
//        p.be_loaded &= ((Session["UserControl_~_binder_PlaceHolder_content"] as string) == "UserControl_uniform_piece_make");
//        }
        }
      else
        {
        ...
        //
        p.be_loaded = false;
        p.be_ok_to_config_uniform_piece_makes = k.Has((string[])(Session["privilege_array"]), "config-uniforms");
        p.id = k.EMPTY;
        p.summary = null;
        }
      }
    ...
    private void InitializeComponent()
      ...
      this.PreRender += this.TWebUserControl_uniform_piece_make_PreRender;
      }

    private void TWebUserControl_uniform_piece_make_PreRender(object sender, System.EventArgs e)
      {
      SessionSet(InstanceId() + ".p", p);
      }

    public TWebUserControl_uniform_piece_make Fresh()
      {
    ...
    protected void Button_delete_Click(object sender, System.EventArgs e)
      {
      if (p.biz_uniform_piece_makes.Delete(k.Safe(TextBox_id.Text, k.safe_hint_type.NUM)))
        {
        SetLookupMode();
        }
      else
    ...
    protected void Button_lookup_Click(object sender, System.EventArgs e)
      ...
      if (!PresentRecord(saved_id))
        ...
        p.biz_uniform_piece_makes.Bind(saved_id, DropDownList_id);
    ...
    internal void SetP
      ...
      if (id.Length > 0)
        {
        p.id = id;
        p.summary = p.biz_uniform_piece_makes.Summary(id);
        //p.be_ok_to_config_uniform_piece_makes = p.biz_privileges.HasForAgency
        //  (
        //  member_id:p.biz_members.IdOfUserId(p.biz_user.IdNum()),
        //  privilege_name:"config-uniform_piece_makes",
        //  agency_id:p.biz_uniform_piece_makes.AgencyIdOf(p.summary)
        //  );
        p.presentation_mode = (p.be_ok_to_config_uniform_piece_makes ? presentation_mode_enum.FULL_FUNCTION : p.presentation_mode = presentation_mode_enum.REVIEW_ONLY);
        }
      else
        {
        p.id = k.EMPTY;
    ...
    } // end TWebUserControl_uniform_piece_make
