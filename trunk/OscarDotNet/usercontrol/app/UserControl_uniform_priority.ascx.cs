...
namespace UserControl_uniform_priority
  {
  public partial class TWebUserControl_uniform_priority: ki_web_ui.usercontrol_class
    {
    ...
    protected void Page_Load(object sender, System.EventArgs e)
      ...
      if (!p.be_loaded)
        {
        LinkButton_new_record.Visible = p.be_ok_to_config_uniform_priorities;
    ...
    private bool PresentRecord(string id)
      ...
      if
        ...
        SetDependentFieldAblements(p.be_ok_to_config_uniform_priorities);
        Button_submit.Enabled = p.be_ok_to_config_uniform_priorities;
        Button_delete.Enabled = p.be_ok_to_config_uniform_priorities;
        result = true;
        }
      return result;
      }
    ...
    protected override void OnInit(System.EventArgs e)
      ...
      if (Session[instance_id + ".p"] != null)
        ...
        //
        if (instance_id == "ASP.protected_overview_aspx.UserControl_M_config_business_objects_binder_uniform_object_binder_uniform_priority")
          {
          p.be_loaded &= ((Session["M_UserControl_config_UserControl_business_objects_binder_UserControl_uniform_object_binder_PlaceHolder_content"] as string) == "UserControl_uniform_priority");
          }
//      else if (instance_id == "ASP.~_aspx.UserControl_~_binder_uniform_priority")
//        {
//        p.be_loaded &= ((Session["UserControl_~_binder_PlaceHolder_content"] as string) == "UserControl_uniform_priority");
//        }
        }
      else
        {
    ...
    public TWebUserControl_uniform_priority Fresh()
      {
    ...
    protected void Button_delete_Click(object sender, System.EventArgs e)
      {
      if (p.biz_uniform_priorities.Delete(k.Safe(TextBox_id.Text, k.safe_hint_type.NUM)))
        {
        SetLookupMode();
        }
      else
    ...
    internal void SetP
      ...
      if (id.Length > 0)
        {
        p.id = id;
        p.summary = p.biz_uniform_priorities.Summary(id);
        //p.be_ok_to_config_uniform_priorities = p.biz_privileges.HasForAgency
        //  (
        //  member_id:p.biz_members.IdOfUserId(p.biz_user.IdNum()),
        //  privilege_name:"config-uniform_priorities",
        //  agency_id:p.biz_uniform_priorities.AgencyIdOf(p.summary)
        //  );
        p.presentation_mode = (p.be_ok_to_config_uniform_priorities ? presentation_mode_enum.FULL_FUNCTION : p.presentation_mode = presentation_mode_enum.REVIEW_ONLY);
        }
      else
        {
        p.id = k.EMPTY;
    ...
    } // end TWebUserControl_uniform_priority
