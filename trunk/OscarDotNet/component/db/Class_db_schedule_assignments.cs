  ...
  public class TClass_db_schedule_assignments: TClass_db
    ...
    internal void BindAmbulanceStaffingTimeLineChartBaseDataList
      ...
      target.DataSource = new MySqlCommand
        ...
        +   " ("
        +   " select nominal_day"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '000000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0000"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '003000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0030"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '010000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0100"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '013000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0130"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '020000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0200"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '023000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0230"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '030000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0300"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '033000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0330"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '040000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0400"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '043000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0430"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '050000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0500"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '053000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0530"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '060000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0600"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '063000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0630"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '070000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0700"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '073000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0730"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '080000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0800"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '083000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0830"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '090000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0900"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '093000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d0930"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '100000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d1000"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '103000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d1030"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '110000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d1100"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '113000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as d1130"
        +   " from schedule_assignment"
        +     " join shift on (shift.id=schedule_assignment.shift_id)"
        +     " join member on (member.id=schedule_assignment.member_id)"
        +     " join medical_release_code_description_map on (medical_release_code_description_map.code=member.medical_release_code)"
        ...
        +   " ("
        +   " select nominal_day"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '000000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0000"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '003000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0030"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '010000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0100"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '013000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0130"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '020000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0200"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '023000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0230"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '030000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0300"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '033000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0330"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '040000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0400"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '043000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0430"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '050000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0500"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '053000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0530"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '060000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0600"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '063000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0630"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '070000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0700"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '073000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0730"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '080000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0800"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '083000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0830"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '090000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0900"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '093000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n0930"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '100000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n1000"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '103000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n1030"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '110000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n1100"
        +   " , FORMAT(SUM(muster_to_logon_timespan <= CAST(@t := '113000' AS TIME) AND CAST(@t AS TIME) < muster_to_logoff_timespan)/2,1) as n1130"
        +   " from schedule_assignment"
        +     " join shift on (shift.id=schedule_assignment.shift_id)"
        +     " join member on (member.id=schedule_assignment.member_id)"
        +     " join medical_release_code_description_map on (medical_release_code_description_map.code=member.medical_release_code)"
