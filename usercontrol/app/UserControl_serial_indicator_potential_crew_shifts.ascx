<%@ Control Language="c#" AutoEventWireup="True" Codebehind="UserControl_serial_indicator_potential_crew_shifts.ascx.cs" Inherits="UserControl_serial_indicator_potential_crew_shifts.TWebUserControl_serial_indicator_potential_crew_shifts"%>
  ...
  <Columns>
	<ASP:TemplateColumn>
	  <ItemTemplate>
		...
		<p><asp:Image id="Image_control" runat="server" width="781" height="417" ImageUrl=<%# string.Format("~/serial_indicator_control_chart.aspx?indicator=crew_shifts_forecast&agency={0}&be_agency_applicable={1}",DataBinder.Eval(Container.DataItem, "id"),DataBinder.Eval(Container.DataItem, "be_agency_id_applicable")) %> AlternateText="crew_shifts_forecast"/></p>
	  </ItemTemplate>
	</ASP:TemplateColumn>
  </Columns>
</ASP:DataGrid>
