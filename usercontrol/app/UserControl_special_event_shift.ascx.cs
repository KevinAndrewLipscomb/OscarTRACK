...
using Class_biz_special_event_shifts;
...
namespace UserControl_special_event_shift
  {
  public partial class TWebUserControl_special_event_shift: ki_web_ui.usercontrol_class
    {
    ...
    private void Clear()
      ...
      TextBox_special_event_id.Text = k.EMPTY;
    ...
    protected void Page_Load(object sender, System.EventArgs e)
      ...
      if (!p.be_loaded)
        {
        LinkButton_new_record.Visible = p.be_ok_to_config_special_event_shifts;
        LinkButton_go_to_match_first.Text = k.ExpandTildePath(LinkButton_go_to_match_first.Text);
        LinkButton_go_to_match_prior.Text = k.ExpandTildePath(LinkButton_go_to_match_prior.Text);
        LinkButton_go_to_match_next.Text = k.ExpandTildePath(LinkButton_go_to_match_next.Text);
        LinkButton_go_to_match_last.Text = k.ExpandTildePath(LinkButton_go_to_match_last.Text);
        RequireConfirmation(Button_delete, "Are you sure you want to delete this record?");
#warning Remove the following line if the special_event_shift will primarily be a reference table as opposed to a log or journal table.
        SetDataEntryMode();
        if ((Session["mode:goto"] != null) && Session["mode:goto"].ToString().Contains("/special_event_shift/"))
          {
    ...
    private bool PresentRecord(string id)
      ...
      if
        (
        p.biz_special_event_shifts.Get
          (
          id,
        ...
        TextBox_special_event_id.Text = special_event_id;
        ...
        SetDependentFieldAblements(p.be_ok_to_config_special_event_shifts);
        Button_submit.Enabled = p.be_ok_to_config_special_event_shifts;
        Button_delete.Enabled = p.be_ok_to_config_special_event_shifts;
        result = true;
        }
      return result;
      }
...
#warning Revise the following line if the usercontrol will not be a direct child of the member binder.
        p.be_loaded = IsPostBack && ((Session["UserControl_member_binder_PlaceHolder_content"] as string) == "UserControl_special_event_shift");
        }
      else
        {
        p.be_loaded = false;
        p.biz_special_event_shifts = new TClass_biz_special_event_shifts();
        p.biz_role_member_map = new TClass_biz_role_member_map();
        p.be_ok_to_config_special_event_shifts = k.Has((string[])(Session["privilege_array"]), "config-special_event_shifts");
        }
      }
    ...
    private void InitializeComponent()
      ...
      this.PreRender += this.TWebUserControl_special_event_shift_PreRender;
      }

    private void TWebUserControl_special_event_shift_PreRender(object sender, System.EventArgs e)
      {
      SessionSet(InstanceId() + ".p", p);
      }

    public TWebUserControl_special_event_shift Fresh()
      {
    ...
    protected void Button_submit_Click(object sender, System.EventArgs e)
      ...
      if (Page.IsValid)
        {
        p.biz_special_event_shifts.Set
          (
          k.Safe(TextBox_id.Text,k.safe_hint_type.NUM),
#warning Safe_hint values should be tightened.
          k.Safe(TextBox_special_event_id.Text,k.safe_hint_type.PUNCTUATED).Trim(),
...
#warning Review safe-hint and tighten if necessary
    ...
    protected void Button_delete_Click(object sender, System.EventArgs e)
      {
      if (p.biz_special_event_shifts.Delete(k.Safe(TextBox_id.Text, k.safe_hint_type.ALPHANUM)))
        {
        SetLookupMode();
        }
      else
    ...
    private void SetDependentFieldAblements(bool ablement)
      {
      TextBox_special_event_id.Enabled = ablement;
    ...
    protected void Button_lookup_Click(object sender, System.EventArgs e)
      ...
      if (!PresentRecord(saved_id))
        ...
        p.biz_special_event_shifts.Bind(saved_id, DropDownList_id);
    ...
    } // end TWebUserControl_special_event_shift
