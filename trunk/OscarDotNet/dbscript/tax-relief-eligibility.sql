...
, PERCENTAGE((select IFNULL(sum(TIME_TO_SEC(TIMEDIFF(muster_to_logoff_timespan,muster_to_logon_timespan))/3600*be_selected),0) from schedule_assignment where member_id = subquery.member_id and YEAR(nominal_day) = 2020),ROUND((IFNULL(BASE_OBLIGATION(jan_code),0) + IFNULL(BASE_OBLIGATION(feb_code),0) + IFNULL(BASE_OBLIGATION(mar_code),0) + IFNULL(BASE_OBLIGATION(apr_code),0) + IFNULL(BASE_OBLIGATION(may_code),0) + IFNULL(BASE_OBLIGATION(jun_code),0) + IFNULL(BASE_OBLIGATION(jul_code),0) + IFNULL(BASE_OBLIGATION(aug_code),0) + IFNULL(BASE_OBLIGATION(sep_code),0) + IFNULL(BASE_OBLIGATION(oct_code),0) + IFNULL(BASE_OBLIGATION(nov_code),0) + IFNULL(BASE_OBLIGATION(dec_code),0))*12,1)) as 2020_pct_of_base
...
, PERCENTAGE((select IFNULL(sum(TIME_TO_SEC(TIMEDIFF(muster_to_logoff_timespan,muster_to_logon_timespan))/3600*be_selected),0) from schedule_assignment where member_id = subquery.member_id and YEAR(nominal_day) = 2020),ROUND(IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-01-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(jan_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-02-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(feb_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-03-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(mar_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-04-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(apr_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-05-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(may_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-06-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(jun_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-07-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(jul_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-08-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(aug_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-09-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(sep_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-10-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(oct_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-11-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(nov_code),0))*12 + IFNULL((select num_obliged_shifts from leave_of_absence where member_id = subquery.member_id and '2020-12-15' between start_date and end_date),IFNULL(BASE_OBLIGATION(dec_code),0))*12,1)) as 2020_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,1) as jan_duty_hours
, ENROLLMENT(jan_code) as jan_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(jan_code)*12,0),1) as jan_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,1),BASE_OBLIGATION(jan_code)*12) as jan_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,1),EFFECTIVE_OBLIGATION(subquery.member_id,2020,1,jan_code)) as jan_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,2) as feb_duty_hours
, ENROLLMENT(feb_code) as feb_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(feb_code)*12,0),1) as feb_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,2),BASE_OBLIGATION(feb_code)*12) as feb_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,2),EFFECTIVE_OBLIGATION(subquery.member_id,2020,2,feb_code)) as feb_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,3) as mar_duty_hours
, ENROLLMENT(mar_code) as mar_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(mar_code)*12,0),1) as mar_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,3),BASE_OBLIGATION(mar_code)*12) as mar_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,3),EFFECTIVE_OBLIGATION(subquery.member_id,2020,3,mar_code)) as mar_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,4) as apr_duty_hours
, ENROLLMENT(apr_code) as apr_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(apr_code)*12,0),1) as apr_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,4),BASE_OBLIGATION(apr_code)*12) as apr_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,4),EFFECTIVE_OBLIGATION(subquery.member_id,2020,4,apr_code)) as apr_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,5) as may_duty_hours
, ENROLLMENT(may_code) as may_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(may_code)*12,0),1) as may_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,5),BASE_OBLIGATION(may_code)*12) as may_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,5),EFFECTIVE_OBLIGATION(subquery.member_id,2020,5,may_code)) as may_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,6) as jun_duty_hours
, ENROLLMENT(jun_code) as jun_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(jun_code)*12,0),1) as jun_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,6),BASE_OBLIGATION(jun_code)*12) as jun_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,6),EFFECTIVE_OBLIGATION(subquery.member_id,2020,6,jun_code)) as jun_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,7) as jul_duty_hours
, ENROLLMENT(jul_code) as jul_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(jul_code)*12,0),1) as jul_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,7),BASE_OBLIGATION(jul_code)*12) as jul_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,7),EFFECTIVE_OBLIGATION(subquery.member_id,2020,7,jul_code)) as jul_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,8) as aug_duty_hours
, ENROLLMENT(aug_code) as aug_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(aug_code)*12,0),1) as aug_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,8),BASE_OBLIGATION(aug_code)*12) as aug_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,8),EFFECTIVE_OBLIGATION(subquery.member_id,2020,8,aug_code)) as aug_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,9) as sep_duty_hours
, ENROLLMENT(sep_code) as sep_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(sep_code)*12,0),1) as sep_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,9),BASE_OBLIGATION(sep_code)*12) as sep_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,9),EFFECTIVE_OBLIGATION(subquery.member_id,2020,9,sep_code)) as sep_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,10) as oct_duty_hours
, ENROLLMENT(oct_code) as oct_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(oct_code)*12,0),1) as oct_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,10),BASE_OBLIGATION(oct_code)*12) as oct_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,10),EFFECTIVE_OBLIGATION(subquery.member_id,2020,10,oct_code)) as oct_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,11) as nov_duty_hours
, ENROLLMENT(nov_code) as nov_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(nov_code)*12,0),1) as nov_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,11),BASE_OBLIGATION(nov_code)*12) as nov_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,11),EFFECTIVE_OBLIGATION(subquery.member_id,2020,11,nov_code)) as nov_pct_of_effective

, DUTY_HOURS(subquery.member_id,2020,12) as dec_duty_hours
, ENROLLMENT(dec_code) as dec_enrollment
, FORMAT(IFNULL(BASE_OBLIGATION(dec_code)*12,0),1) as dec_base_obligation
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,12),BASE_OBLIGATION(dec_code)*12) as dec_pct_of_base
...
, PERCENTAGE(DUTY_HOURS(subquery.member_id,2020,12),EFFECTIVE_OBLIGATION(subquery.member_id,2020,12,dec_code)) as dec_pct_of_effective
from
  (
