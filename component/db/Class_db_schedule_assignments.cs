    ...
    internal void Update
      ...
      try
        ...
        for (var i = new k.subtype<int>(0,31); i.val < i.LAST; i.val++)
          {
          sql += k.EMPTY
          + " insert schedule_assignment (nominal_day,shift_id,post_id,member_id,be_selected,be_new)"
          ...
          + " , @post_id := (select id from agency where oscar_classic_enumerator = coord_agency) as post_id"
          + " , odnmid as member_id"
          + " , @be_selected := (@post_id = (select agency_id from member where id = odnmid)) as be_selected"
