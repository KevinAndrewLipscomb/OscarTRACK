...
-- Tested with MySQL 5.1 on Ubuntu 6.01DELIMITER $$
-- Updated Nov 27, 2007 to fix a bug in the 'CC' section
DELIMITER $$
DROP FUNCTION IF EXISTS `DOUBLE_METAPHONE` $$
CREATE FUNCTION `DOUBLE_METAPHONE`(st VARCHAR(55)) RETURNS varchar(128)
    NO SQL
BEGIN
	...
	SET first = 3;
	SET st = CONCAT(REPEAT('-', first -1), UCASE(st), '-----'); --  pad st so we can index beyond the begining AND end of the input string
    ...
    SET prevpos = pos;
		...
		WHEN ch = 'B' THEN
			...
			IF SUBSTRING(st, pos+1, 1) = 'B' THEN
				SET pri = CONCAT(pri, 'P'), pos = pos  + 2; -- nxt = ('P', 2)
			ELSE
				SET pri = CONCAT(pri, 'P'), pos = pos  + 1; -- nxt = ('P', 1)
			END IF;
		WHEN ch = 'C' THEN
			--  various germanic
			IF (pos > first AND SUBSTRING(st, pos-2, 1) IN ('A', 'E', 'I', 'O', 'U', 'Y') AND SUBSTRING(st, pos-1, 3) = 'ACH' AND
		...
		WHEN ch = 'G' THEN
			IF SUBSTRING(st, pos+1, 1) = 'H' THEN
				...
				ELSEIF (pos > (first + 1) AND SUBSTRING(st, pos-2, 1) IN ('B', 'H', 'D') )
				   ...
				   OR (pos > (first + 3) AND SUBSTRING(st, pos-3, 1) IN ('B', 'H') ) THEN
				...
				ELSE
					...
					ELSE
						IF SUBSTRING(st, pos+1, 1) not IN ('L', 'T', 'K', 'S', 'N', 'M', 'B', 'Z')
						   AND SUBSTRING(st, pos-1) not IN ('S', 'K', 'L') THEN
		...
		WHEN ch = 'S' THEN
			...
			ELSEIF (pos = first AND SUBSTRING(st, pos+1, 1) IN ('M', 'N', 'L', 'W')) OR SUBSTRING(st, pos+1, 1) = 'Z' THEN
				...
				ELSE
					SET pos = pos + 1;
				END IF;
			ELSEIF SUBSTRING(st, pos+2, 2) = 'SC' THEN
				--  Schlesinger's rule
		...
		WHEN ch = 'X' THEN
			...
			IF not(pos = last AND (SUBSTRING(st, pos-3, 3) IN ('IAU', 'EAU')
			   OR SUBSTRING(st, pos-2, 2) IN ('AU', 'OU'))) THEN
				SET sec = CONCAT(sec, 'KS'), pos = pos  + 1; -- nxt = ('KS',)
			END IF;
