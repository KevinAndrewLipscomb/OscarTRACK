...
namespace Class_db_patient_age_units
  ...
  public class TClass_db_patient_age_units: TClass_db
    ...
    public void BindDirectToListControl
      ...
      Open();
      var dr = new MySqlCommand("SELECT id,description FROM patient_age_unit where description <> '(none specified)' order by id", connection).ExecuteReader();
      while (dr.Read())
        {
    ...
    public bool Delete(string id)
      ...
      try
        {
        new MySqlCommand(db_trail.Saved("delete from patient_age_unit where id = '" + id + "'"), connection).ExecuteNonQuery();
        }
    ...
    public bool Get
      ...
      var result = false;
      Open();
      var dr = new MySqlCommand("select description from patient_age_unit where id = '" + id + "'", connection).ExecuteReader();
      if (dr.Read())
        {
    ...
    internal string IdOfDescription(string description)
      ...
      Open();
      id_of_description = new MySqlCommand("select id from patient_age_unit where description = '" + description + "'",connection).ExecuteScalar().ToString();
      Close();
