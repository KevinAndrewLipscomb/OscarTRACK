...
using Class_biz_uniform_issuances;
...
namespace UserControl_uniform_issuance
  {
  public partial class TWebUserControl_uniform_issuance: ki_web_ui.usercontrol_class
    {
    private struct p_type
      {
      public bool be_loaded;
      public TClass_biz_uniform_issuances biz_uniform_issuances;
      public TClass_biz_role_member_map biz_role_member_map;
      public bool be_ok_to_config_uniform_issuances;
      public string id;
    ...
    private void Clear()
      ...
      UserControl_drop_down_date_date.Clear();
      TextBox_catalog_id.Text = k.EMPTY;
    ...
    protected void Page_Load(object sender, System.EventArgs e)
      ...
      if (!p.be_loaded)
        {
        LinkButton_new_record.Visible = p.be_ok_to_config_uniform_issuances;
        ...
        if (p.presentation_mode == presentation_mode_enum.NEW)
          {
#warning Remove the following line if the uniform_issuance will primarily be a reference table as opposed to a log or journal table.
          SetDataEntryMode();
          }
        else
          {
    ...
    private bool PresentRecord(string id)
      ...
      if
        ...
        UserControl_drop_down_date_date.selectedvalue = date;
        TextBox_catalog_id.Text = catalog_id;
        Button_lookup.Enabled = false;
        Label_lookup_arrow.Enabled = false;
        Label_lookup_hint.Enabled = false;
        LinkButton_reset.Enabled = true;
        SetDependentFieldAblements(p.be_ok_to_config_uniform_issuances);
        Button_submit.Enabled = p.be_ok_to_config_uniform_issuances;
        Button_delete.Enabled = p.be_ok_to_config_uniform_issuances;
        result = true;
        }
      return result;
      }
...
#warning Revise the binder-related instance_id to this control appropriately.
        if (instance_id == "ASP.protected_overview_aspx.UserControl_member_binder_uniform_issuance")
          {
#warning Revise the ClientID path to this control appropriately.
          p.be_loaded &= ((Session["UserControl_member_binder_PlaceHolder_content"] as string) == "UserControl_uniform_issuance");
          }
//      else if (instance_id == "ASP.~_aspx.UserControl_~_binder_uniform_issuance")
//        {
//        p.be_loaded &= ((Session["UserControl_~_binder_PlaceHolder_content"] as string) == "UserControl_uniform_issuance");
//        }
        }
      else
        {
        p.biz_uniform_issuances = new TClass_biz_uniform_issuances();
        p.biz_role_member_map = new TClass_biz_role_member_map();
        //
        p.be_loaded = false;
        p.be_ok_to_config_uniform_issuances = k.Has((string[])(Session["privilege_array"]), "config-uniform_issuances");
        p.id = k.EMPTY;
        p.summary = null;
        }
      }
    ...
    private void InitializeComponent()
      {
      PreRender += TWebUserControl_uniform_issuance_PreRender;
      }

    private void TWebUserControl_uniform_issuance_PreRender(object sender, System.EventArgs e)
      {
      SessionSet(InstanceId() + ".p", p);
      }

    public TWebUserControl_uniform_issuance Fresh()
      {
      Session.Remove(InstanceId() + ".p");
      UserControl_drop_down_date_date.Fresh();
      return this;
      }
    ...
    protected void Button_submit_Click(object sender, System.EventArgs e)
      ...
      if (Page.IsValid)
        {
        p.biz_uniform_issuances.Set
          (
          k.Safe(TextBox_id.Text,k.safe_hint_type.NUM),
#warning Safe_hint values should be tightened.
          k.Safe(TextBox_member_id.Text,k.safe_hint_type.PUNCTUATED).Trim(),
          UserControl_drop_down_date_date.selectedvalue,
          k.Safe(TextBox_catalog_id.Text,k.safe_hint_type.PUNCTUATED).Trim()
          );
...
#warning Review safe-hint and tighten if necessary
    ...
    protected void Button_delete_Click(object sender, System.EventArgs e)
      {
      if (p.biz_uniform_issuances.Delete(k.Safe(TextBox_id.Text, k.safe_hint_type.ALPHANUM)))
        {
        SetLookupMode();
        }
      else
    ...
    private void SetDependentFieldAblements(bool ablement)
      ...
      UserControl_drop_down_date_date.enabled = ablement;
      TextBox_catalog_id.Enabled = ablement;
      }
    ...
    protected void Button_lookup_Click(object sender, System.EventArgs e)
      ...
      if (!PresentRecord(saved_id))
        ...
        p.biz_uniform_issuances.Bind(saved_id, DropDownList_id);
    ...
    internal void SetP
      ...
      if (id.Length > 0)
        {
        p.id = id;
        p.summary = p.biz_uniform_issuances.Summary(id);
        //p.be_ok_to_config_uniform_issuances = p.biz_privileges.HasForContainer
        //  (
        //  member_id:p.biz_members.IdOfUserId(p.biz_user.IdNum()),
        //  privilege_name:"config-uniform_issuances",
        //  container_id:p.biz_uniform_issuances.ContainerIdOf(p.summary)
        //  );
        p.presentation_mode = (p.be_ok_to_config_uniform_issuances ? presentation_mode_enum.FULL_FUNCTION : p.presentation_mode = presentation_mode_enum.REVIEW_ONLY);
        }
      else
        {
        p.id = k.EMPTY;
    ...
    } // end TWebUserControl_uniform_issuance
