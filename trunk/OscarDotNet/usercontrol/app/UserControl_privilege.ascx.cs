...
namespace UserControl_privilege
  {

  public partial class TWebUserControl_privilege: ki_web_ui.usercontrol_class
    {
    ...
    private struct p_type
      ...
      public bool be_ok_to_config_privileges;
      ...
      public string[] user_privilege_string_array;
    ...
    private void Page_Load(object sender, EventArgs e)
      ...
      if (!p.be_loaded)
        ...
        if (Session["mode:report"] == null)
          ...
          if (p.be_ok_to_config_privileges)
            {
        ...
        Lookup(p.user_privilege_string_array[0]);
        p.be_loaded = true;
        }
      }
    ...
    private bool PresentRecord(string name)
      ...
      if (p.biz_privileges.Get(name))
        ...
        SetDependentFieldAblements(p.be_ok_to_config_privileges);
        Button_submit.Visible = p.be_ok_to_config_privileges;
        Button_delete.Visible = p.be_ok_to_config_privileges;
    ...
    private void SetLookupMode()
      ...
      LinkButton_new_record.Enabled = p.be_ok_to_config_privileges;
    ...
    protected override void OnInit(EventArgs e)
      ...
      if (Session[instance_id + ".p"] != null)
        ...
        //p.be_loaded = IsPostBack && ((Session["M_UserControl_config_UserControl_privileges_and_matrices_binder_PlaceHolder_content"] as string) == "UserControl_privilege");
        //
        // If this control is being used dynamically under one or more parent binder(s), it must ascertain which instance it is, and whether or not that instance's parent binder
        // had it loaded already.
        //
        if (instance_id == "ASP.protected_overview_aspx.UserControl_M_config_roles_and_matrices_binder_privilege")
          {
          p.be_loaded &= ((Session["M_UserControl_config_UserControl_roles_and_matrices_binder_PlaceHolder_content"] as string) == "UserControl_privilege");
          }
        }
      else
        {
        ...
        p.be_ok_to_config_privileges = false; // configuring privileges requires programming changes
        p.be_scope_cross_agency = true;
        p.be_sort_order_ascending = true;
        p.num_gridview_rows = 0;
        p.privilege_name = k.EMPTY;
        p.sort_order = "member_name%";
        p.user_privilege_string_array = p.biz_user.Privileges();
    ...
    private void InitializeComponent()
      ...
      PreRender += TWebUserControl_privilege_PreRender;
      }

    private void TWebUserControl_privilege_PreRender(object sender, EventArgs e)
      {
      SessionSet(InstanceId() + ".p", p);
      }

    public TWebUserControl_privilege Fresh()
      {
    ...
    protected void DropDownList_name_SelectedIndexChanged(object sender, EventArgs e)
      {
      p.privilege_name = k.Safe(DropDownList_name.SelectedValue, k.safe_hint_type.PUNCTUATED);
      PresentRecord(p.privilege_name);
      }
    ...
    protected void Button_delete_Click(object sender, EventArgs e)
      {
      //if (p.biz_privileges.Delete(k.Safe(TextBox_name.Text, k.safe_hint_type.HUMAN_NAME)))
      //  {
      //  SetLookupMode();
      //  }
      //else
    ...
    protected void LinkButton_new_record_Click(object sender, EventArgs e)
      {
      Clear();
      TextBox_name.Enabled = p.be_ok_to_config_privileges;
      Button_lookup.Enabled = false;
      Label_lookup_arrow.Enabled = false;
      Label_lookup_hint.Enabled = false;
      LinkButton_reset.Enabled = true;
    ...
    private void Lookup(string initial_selection = k.EMPTY)
      ...
      if (initial_selection.Length > 0)
        {
        p.privilege_name = initial_selection;
        DropDownList_name.Items.FindByText(p.privilege_name).Selected = true;
        PresentRecord(p.privilege_name);
        }
      }
    ...
    private void BindHolders()
      ...
      p.biz_privileges.AnalyzeForPrivilegeComms
        (
        user_privilege_string_array: p.user_privilege_string_array,
        selected_privilege_name: p.privilege_name,
        be_scope_cross_agency: ref p.be_scope_cross_agency,
        be_privilege_held_by_user: out be_privilege_held_by_user,
      ...
      p.biz_role_member_map.BindPrivilegeHolders
        (
      ...
      Table_quick_message.Visible = p.biz_privileges.BeOkToAllowPrivilegeCommsQuickMessaging
        (
        has_user_quickmessage_priv:k.Has((string[])(Session["privilege_array"]), "send-quickmessages"),
        be_privilege_held_by_user:be_privilege_held_by_user,
        be_any_privilege_holders:!p.be_gridview_empty
        );
      //
