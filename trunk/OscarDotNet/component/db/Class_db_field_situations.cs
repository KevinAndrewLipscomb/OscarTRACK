  ...
  public class TClass_db_field_situations: TClass_db
    ...
    internal Queue<digest> DigestQ()
      ...
      var dr = new MySqlCommand
        ...
        +     " ("
        +     " SELECT call_sign"
        +     " , IF(call_sign REGEXP '^[[:digit:]]'," // ambulances
        +          " REPLACE("
        +             " REPLACE("
        +                " REPLACE("
        +                   " REPLACE("
        ...
        +                " 'P',''),"
        +             " 'D',''),"
        +          " IF(call_sign REGEXP '(^E[[:digit:]])|(^L[[:digit:]])|(^FRSQ[[:digit:]])'," // engines, ladders, frsqs
        +             " REPLACE(call_sign,'P',''),"
        +             " call_sign"
        +             " )"
        +          " )"
        +       " as call_sign_unqualified"
        +     " , max(incident_num) as max_incident_num"
        +     " FROM cad_record"
        +     " where call_sign not in ('ACPI','ARSN','EYES','FAST','FIGP','MISC','NBRU','OVD2','SICK','UNCO')"
        +     " group by call_sign_unqualified"
        +     " )"
        +     " as reduced"
