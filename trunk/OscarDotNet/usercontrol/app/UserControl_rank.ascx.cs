...
namespace UserControl_rank
  {
  public partial class TWebUserControl_rank: ki_web_ui.usercontrol_class
    {
    ...
    protected void Page_Load(object sender, System.EventArgs e)
      ...
      if (!p.be_loaded)
        {
        LinkButton_new_record.Visible = p.be_ok_to_config_ranks;
    ...
    private bool PresentRecord(string id)
      ...
      if
        ...
        SetDependentFieldAblements(p.be_ok_to_config_ranks);
        Button_submit.Enabled = p.be_ok_to_config_ranks;
        Button_delete.Enabled = p.be_ok_to_config_ranks;
        result = true;
        }
      return result;
      }
    ...
    protected override void OnInit(System.EventArgs e)
      ...
      if (Session[instance_id + ".p"] != null)
        ...
        //
        if (instance_id == "ASP.protected_overview_aspx.UserControl_M_config_business_objects_binder_uniform_object_binder_rank")
          {
          p.be_loaded &= ((Session["M_UserControl_config_UserControl_business_objects_binder_UserControl_uniform_object_binder_PlaceHolder_content"] as string) == "UserControl_rank");
          }
//      else if (instance_id == "ASP.~_aspx.UserControl_~_binder_rank")
//        {
//        p.be_loaded &= ((Session["UserControl_~_binder_PlaceHolder_content"] as string) == "UserControl_rank");
//        }
        }
      else
        {
    ...
    private void InitializeComponent()
      ...
      this.PreRender += this.TWebUserControl_rank_PreRender;
      }

    private void TWebUserControl_rank_PreRender(object sender, System.EventArgs e)
      {
      SessionSet(InstanceId() + ".p", p);
      }

    public TWebUserControl_rank Fresh()
      {
    ...
    protected void Button_delete_Click(object sender, System.EventArgs e)
      {
      if (p.biz_ranks.Delete(k.Safe(TextBox_id.Text, k.safe_hint_type.NUM)))
        {
        SetLookupMode();
        }
      else
    ...
    internal void SetP
      ...
      if (id.Length > 0)
        {
        p.id = id;
        p.summary = p.biz_ranks.Summary(id);
        //p.be_ok_to_config_ranks = p.biz_privileges.HasForAgency
        //  (
        //  member_id:p.biz_members.IdOfUserId(p.biz_user.IdNum()),
        //  privilege_name:"config-ranks",
        //  agency_id:p.biz_ranks.AgencyIdOf(p.summary)
        //  );
        p.presentation_mode = (p.be_ok_to_config_ranks ? presentation_mode_enum.FULL_FUNCTION : p.presentation_mode = presentation_mode_enum.REVIEW_ONLY);
        }
      else
        {
        p.id = k.EMPTY;
