...
begin
  ...
  if session['UserControl_roster.p'] <> nil then begin
    ...
    p.be_loaded := IsPostBack
      and (string(session['UserControl_member_binder_PlaceHolder_content']) = 'UserControl_personnel_binder')
        // only necessary because the roster is a special case that is deliberately not always freshened by its parent binder
      and (string(session['UserControl_member_binder_UserControl_personnel_binder_PlaceHolder_content']) = 'UserControl_roster');
  end else begin
    //
    p.be_loaded := FALSE;
