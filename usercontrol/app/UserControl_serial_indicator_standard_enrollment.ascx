...
<p>These control charts show the percentage of released core operations personnel who the agency enrolled at one of the standard levels (Regular, Life, Senior, Tenured BLS, Tenured ALS, EDP).&nbsp; The lower the data point, the more personnel the agency enrolled at liberal levels (Associate, Reduced, Staff, ALS Intern, College, Atypical).&nbsp; Standard enrollees commit more duty shifts per month than liberal enrollees.</p>
<table>
  <tr>
    <td nowrap="nowrap">
      <b>Width in months:</b>
  ...
  <Columns>
	<ASP:TemplateColumn>
	  <ItemTemplate>
		...
		<p><asp:Image id="Image_control" runat="server" width="781" height="417" ImageUrl=<%# string.Format("~/serial_indicator_control_chart.aspx?indicator=standard_enrollment&agency={0}&be_agency_applicable={1}",DataBinder.Eval(Container.DataItem, "id"),DataBinder.Eval(Container.DataItem, "be_agency_id_applicable")) %> AlternateText="standard_enrollment"/></p>
	  </ItemTemplate>
	</ASP:TemplateColumn>
  </Columns>
</ASP:DataGrid>
